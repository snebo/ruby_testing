...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'returns teh average of the min and max values' do
      it 'returns the average' do
        expect(game_guessing.make_guess).to eql(4)
      end
    end
    context 'when min is 0 and max is 9' do
      subject(:game_guessing) {described_class.new(0, 9, number_guessing)}
      it 'returns 4' do
        avg = game_guessing.make_guess
        expect(avg).to eq(4)
      end
    end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:out_of_guess) { described_class.new(0, 9, number_end, 4) }

      it 'is game over' do
        expect(out_of_guess).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      ...
      subject (:out_of_guess) { described_class.new(0, 9, number_end, 5) }
      it 'is not game over' do
        expect(out_of_guess).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:edge_case) { described_class.new(5, 8, number_range, 7)}
      it 'updates min to the same value as max' do
        edge_case.update_range
        expect(edge_case.min).to eq(edge_case.max)
      end

      it 'does not update max' do
        edge_case.update_range
        expect(edge_case.max).to eq(8)
      end
    end
  end
end
