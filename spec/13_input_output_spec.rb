...
RSpec.describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_end) {described_class.new(2, '5')}
      # remove the 'x' before running this test
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        count = game_three.count
        congrats_phrase = "Congratulations! You picked the random number in #{count} guesses!\n"
    ...
    # Write a test for the following context.
    subject(:game_4_above) { described_class.new(3, '3', 5) }
    context 'when count is 4 and over' do
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        content = "That was hard. It took you 5 guesses!\n"
        expect {game_4_above.final_message}.to output(content).to_stdout
      end
    end
  end
end
